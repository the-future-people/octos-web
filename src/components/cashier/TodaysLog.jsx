...
export default function TodaysLog() {
  ...
  if (shiftData?.acknowledged_at || shiftData?.opening_float != null) {
    events.push({
      type     : 'SHIFT_START',
      time     : shiftData?.acknowledged_at || null,
      title    : 'Shift started',
            ...
            {events.map((event, idx) => {
              ...
              return (
                <div key={idx} className="flex gap-3 items-start">
                  ...
                  {/* Content */}
                  <div className={`flex-1 min-w-0 rounded-xl border px-3 py-2.5 ${style.color}`}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-semibold text-[var(--text)] break-words leading-snug">
                          {event.title}
                        </div>
                        <div className="text-xs text-[var(--text-3)] mt-0.5 break-words leading-snug">
                          {event.subtitle}
                        </div>
                      </div>
                      <div className="text-right shrink-0 ml-2">
                        {event.amount && (
                          <div className="font-mono font-bold text-sm text-[var(--text)] whitespace-nowrap">
                            {fmt(event.amount)}
                          </div>
                        )}
                        <div className="text-[10px] text-[var(--text-3)] mt-0.5 whitespace-nowrap">
                          {timeStr(event.time)}
                        </div>
                      </div>
                    </div>
